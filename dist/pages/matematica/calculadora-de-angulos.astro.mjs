...
const $$CalculadoraDeAngulos = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Calculadora de \xC1ngulos | Grados, Radianes y Complementarios | Chieffin", "description": "Herramienta gratuita para calcular \xE1ngulos complementarios, suplementarios y convertir grados a radianes de forma f\xE1cil." }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="max-w-4xl mx-auto px-4"> <header class="text-center mb-12"> <span class="bg-indigo-100 text-indigo-700 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest">Geometría y Diseño</span> <h1 class="text-4xl md:text-6xl font-black text-slate-900 mt-6 mb-4 tracking-tight">
...
Calcular Ángulos
</button> <div id="resBoxAng" class="hidden space-y-4"> ${renderComponent($$result2, "ResultBox", $$ResultBox, { "id": "resRelacionados", "label": "\xC1ngulos Relacionados" })} <div class="p-4 bg-indigo-50 rounded-2xl text-center"> <p class="text-indigo-700 font-bold text-sm">En Radianes:</p> <p id="totalRad" class="text-2xl font-black text-indigo-900"></p> </div> </div> </div> <article class="prose prose-slate"> <h2 class="text-2xl font-black text-slate-800 mb-4">
