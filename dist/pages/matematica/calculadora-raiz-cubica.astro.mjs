...
const $$CalculadoraRaizCubica = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Calculadora de Ra\xEDz C\xFAbica Online | Resultados Precisos", "description": "Calcul\xE1 la ra\xEDz c\xFAbica de cualquier n\xFAmero de forma instant\xE1nea. Herramienta matem\xE1tica gratuita con soporte para n\xFAmeros negativos." }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="max-w-4xl mx-auto px-4 py-12"> <div class="text-center mb-10"> <h1 class="text-4xl font-black text-slate-900 mb-4 tracking-tight">
...
</p> </div> <div class="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 mb-12"> <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center"> <div class="space-y-6"> <div> <label class="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Número (Radicando)</label> <div class="relative group"> <span class="absolute left-4 top-1/2 -translate-y-1/2 text-3xl text-slate-300 font-light group-focus-within:text-cyan-500 transition-colors"> <sup class="text-xs font-bold -mr-1">3</sup>√
</span> <input type="number" id="numberInput" class="w-full p-5 pl-14 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-cyan-500 focus:bg-white outline-none transition-all font-mono text-2xl text-slate-800" placeholder="Ej: 27 o -8"> </div> </div> </div> <div class="text-center p-10 bg-cyan-50 rounded-[2.5rem] border border-cyan-100 relative overflow-hidden"> <div class="relative z-10"> <span class="block text-[10px] font-bold uppercase text-cyan-600 mb-2 tracking-widest">Resultado (Raíz)</span> <div id="resultOutput" class="text-6xl font-black text-cyan-700 leading-none">
0
...
La raíz cúbica de un número $x$ es aquel número $y$ que, al ser elevado
        a la potencia de 3 ($y^3$), resulta en $x$. A diferencia de la raíz
        cuadrada, la raíz cúbica está definida para todos los números reales,
        incluyendo los negativos.
</p> <div class="bg-slate-900 p-8 rounded-3xl text-center my-8 shadow-2xl"> <span class="text-cyan-400 font-mono text-3xl">$$\\sqrt[3]${x} = y \\iff y^3 = x$$</span> </div> <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12"> <div class="p-6 bg-slate-50 rounded-2xl border border-slate-200"> <h4 class="font-bold text-slate-900 mb-2">Cálculo con Negativos</h4> <p class="text-xs leading-relaxed">
Si calculas $\\sqrt[3]${ -8}$, el resultado es $-2$, porque $(-2)
            \\times (-2) \\times (-2) = -8$. Esto es posible porque el exponente
            es impar.
...
</p> </div> </div> </section> </main> ` })} ${renderScript($$result, "C:/proyectos/seo-tools/src/pages/matematica/calculadora-raiz-cubica.astro?astro&type=script&index=0&lang.ts")}`;
