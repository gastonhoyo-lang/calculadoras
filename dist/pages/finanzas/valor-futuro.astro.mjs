...
const $$ValorFuturo = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Calculadora de Valor Futuro | Proyecci\xF3n de Inversiones", "description": "Calcul\xE1 cu\xE1nto valdr\xE1 tu inversi\xF3n en el futuro. Proyect\xE1 el crecimiento de tu capital con inter\xE9s compuesto y aportes mensuales." }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="max-w-4xl mx-auto px-4 py-12"> <div class="text-center mb-10"> <h1 class="text-4xl font-black text-slate-900 mb-4 tracking-tight">
Valor Futuro (VF)
</h1> <p class="text-slate-600 max-w-2xl mx-auto">
Proyectá el crecimiento de tu patrimonio basándote en el tiempo y la
        tasa de retorno.
</p> </div> <div class="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 mb-12"> <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center"> <div class="space-y-6"> <div> <label class="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Inversión Inicial ($)</label> <input type="number" id="pvInput" class="w-full p-4 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-lime-500 outline-none transition-all font-mono text-xl" placeholder="Ej: 5000"> </div> <div class="grid grid-cols-2 gap-4"> <div> <label class="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Tasa Anual (%)</label> <input type="number" id="rateInput" class="w-full p-4 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-lime-500 outline-none transition-all font-mono" placeholder="8"> </div> <div> <label class="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Años</label> <input type="number" id="yearsInput" class="w-full p-4 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-lime-500 outline-none transition-all font-mono" placeholder="10"> </div> </div> <div> <label class="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Aporte Mensual ($)</label> <input type="number" id="pmtInput" class="w-full p-4 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-lime-500 outline-none transition-all font-mono" placeholder="Opcional"> </div> </div> <div class="text-center p-10 bg-lime-500 rounded-[2.5rem] shadow-lg shadow-lime-200 relative overflow-hidden h-full flex flex-col justify-center"> <div class="relative z-10 text-white"> <span class="block text-[10px] font-bold uppercase opacity-80 mb-2 tracking-widest">Monto Final Estimado</span> <div id="fvOutput" class="text-6xl font-black leading-none">$0</div> <div class="mt-8 p-4 bg-white/20 rounded-2xl backdrop-blur-sm inline-block"> <span id="interestEarned" class="text-sm font-bold">Ganancia por intereses: $0</span> </div> </div> <span class="absolute -right-4 -bottom-10 text-[12rem] font-black text-white/10 select-none">VF</span> </div> </div> </div> <section class="prose prose-slate max-w-none border-t border-slate-100 pt-12 text-slate-700"> <h2 class="text-3xl font-black text-slate-900 mb-6">
