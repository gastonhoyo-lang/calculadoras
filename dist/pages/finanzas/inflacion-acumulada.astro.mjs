...
Ingresá los porcentajes de cada periodo para calcular el impacto total
        en el costo de vida.
</p> </div> <div class="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 mb-12"> <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-start"> <div class="space-y-6"> <div id="inputsContainer" class="space-y-4"> <label class="block text-[10px] font-bold uppercase text-slate-400 ml-1 tracking-widest">Tasas de Inflación (%)</label> <div class="flex gap-2"> <input type="number" step="any" class="inflation-input w-full p-4 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-red-500 outline-none transition-all font-mono" placeholder="Ej: 4.2"> </div> <div class="flex gap-2"> <input type="number" step="any" class="inflation-input w-full p-4 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-red-500 outline-none transition-all font-mono" placeholder="Ej: 3.8"> </div> </div> <button id="addInput" class="w-full py-3 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 text-xs font-bold hover:bg-slate-50 hover:border-slate-300 transition-all uppercase tracking-widest">
+ Añadir otro mes / periodo
</button> </div> <div class="text-center p-10 bg-red-600 rounded-[2.5rem] shadow-lg shadow-red-200 relative overflow-hidden h-full flex flex-col justify-center"> <div class="relative z-10 text-white"> <span class="block text-[10px] font-bold uppercase opacity-80 mb-2 tracking-widest">Total Acumulado</span> <div id="resultOutput" class="text-7xl font-black leading-none">
0%
...
Esta herramienta te permite saber cuánto debería haber aumentado tu
            ...
            dado.
</p> </div> </div> </section> </main> ` })} ${renderScript($$result, "C:/proyectos/seo-tools/src/pages/finanzas/inflacion-acumulada.astro?astro&type=script&index=0&lang.ts")}`;
